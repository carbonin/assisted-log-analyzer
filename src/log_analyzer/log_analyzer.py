...
class LogAnalyzer:
    ...
    def metadata(self) -> Dict[str, Any]:
        ...
        if self._metadata is None:
            try:
                ...
                raw_metadata = json.loads(metadata_content)
    ...
    def _clean_metadata_json(md: Dict[str, Any]) -> Dict[str, Any]:
        ...
        md["cluster"]["deleted_hosts"] = [h for h in all_hosts if host_deleted_before_installation_started(h)]
        md["cluster"]["hosts"] = [h for h in all_hosts if not host_deleted_before_installation_started(h)]
    ...
    def get_last_install_cluster_events(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to load cluster events: {e}")
    ...
    def get_all_cluster_events(self) -> List[Dict[str, Any]]:
        ...
        if self._cluster_events is None:
            try:
                events_content = self.logs_archive.get(f"cluster_events.json")
                all_events = json.loads(events_content)
    ...
    @staticmethod
    def partition_cluster_events(events: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
    ...
    def get_host_log_file(self, host_id: str, filename: str) -> str:
        ...
        try:
            ...
            logger.debug(f"Found logs under new location: {new_logs_path}")
            return content
        ...
        try:
            ...
            logger.debug(f"Found logs under old location: {old_logs_path}")
            return content
        except FileNotFoundError:
            raise FileNotFoundError(f"Could not find {filename} for host {host_id}")
    ...
    def get_journal_log(self, host_ip: str, journal_file: str, **kwargs) -> str:
        ...
        try:
            ...
            logger.debug(f"Found journal under new location: {new_logs_path}")
            return content
        except FileNotFoundError:
            pass
        ...
        try:
            ...
            logger.debug(f"Found journal under old location: {old_logs_path}")
            return content
        except FileNotFoundError:
            raise FileNotFoundError(f"Could not find {journal_file} for host {host_ip}")

    def get_controller_logs(self) -> str:
        """Get assisted installer controller logs."""
        try:
            return self.logs_archive.get("controller_logs.tar.gz/assisted-installer-controller*.logs")
        except FileNotFoundError:
            raise FileNotFoundError("Could not find controller logs")
