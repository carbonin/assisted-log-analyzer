    ...
    try:
        ...
        if specific_signatures:
            ...
            for sig_name in specific_signatures:
                if sig_name in signature_classes:
                    ...
                    logger.warning(f"Unknown signature: {sig_name}")
        ...
        for signature_class in signatures_to_run:
            ...
            try:
                ...
                logger.error(f"Error running signature {signature_class.__name__}: {e}")
...
def print_results(results: List[SignatureResult]) -> None:
    ...
    print(f"OpenShift Assisted Installer Log Analysis")
    print("=" * 50)
    print()
...
def main() -> int:
    ...
    parser.add_argument(
        "--api-url", default="https://api.openshift.com", help="Base URL for the OpenShift API (default: %(default)s)"
    )
