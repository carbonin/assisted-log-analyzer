...
class SlowImageDownloadSignature(ErrorSignature):
    ...
    def analyze(self, log_analyzer) -> Optional[SignatureResult]:
        ...
        try:
            ...
            logger.error(f"Error in SlowImageDownloadSignature: {e}")

        return None

    @classmethod
    def _list_image_download_info(cls, events: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        ...
        return [info for event in events if (info := get_image_download_info(event)) is not None]
...
class OSInstallationTimeSignature(Signature):
    ...
    def analyze(self, log_analyzer) -> Optional[SignatureResult]:
        ...
        try:
            ...
            logger.error(f"Error in OSInstallationTimeSignature: {e}")

        return None

    @classmethod
    def _create_host_entry(cls, host: Dict[str, Any], host_events: List[Dict[str, Any]], log_analyzer) -> Dict[str, Any]:
        ...
        if total_duration_seconds > cls.slow_threshold_seconds:
            entry["duration"] = f"OS installation was rather slow, it took {total_duration_seconds} seconds"
        else:
            entry["duration"] = f"OS installation was relatively okay, it took {total_duration_seconds} seconds"
...
class InstallationDiskFIOSignature(Signature):
    ...
    def analyze(self, log_analyzer) -> Optional[SignatureResult]:
        ...
        try:
            ...
            logger.error(f"Error in InstallationDiskFIOSignature: {e}")
