...
class AgentStepFailureSignature(Signature):
            ...
            for host in cluster["hosts"]:
                ...
                for _failure_idx, step_failure_log_match in zip(
                    ...
                    if step_failure_message_match is None:
                        logger.warning("Step failure signature skipped failure due to unexpected format")
                        continue
                ...
                if failures:
                    ...
                    report_sections.append(
                        f"Agent step failures for {host_id} ({hostname}):\n"
                        f"{self.generate_table(failures)}"
                    )

            if report_sections:
                return SignatureResult(
                    signature_name=self.name,
                    title="Agent Step Failures",
                    content="\n\n".join(report_sections),
                    severity="error"
                )

        except Exception as e:
            logger.error(f"Error in AgentStepFailureSignature: {e}")

        return None

    @classmethod
    def _filter_message(cls, msg: Dict[str, str]) -> bool:
    ...
    @classmethod
    def _prepare_output(cls, output: str) -> str:
        ...
        output_lines = [line[:cls.MAX_LINE_LENGTH] for line in output_lines]
